...
/** Individual pricing card with its own tilt instance */
const PricingCard = ({ pkg, i }: { pkg: typeof featured[number]; i: number }) => {
  ...
  return (
    <div
      ...
      className={`card-spotlight relative flex flex-col overflow-hidden rounded-2xl shadow-sm ${
        pkg.dark
          ? "card-spotlight-dark bg-gradient-to-br from-[#071629] to-[#1a4a82] hover:shadow-[0_20px_56px_-12px_rgba(26,74,130,0.6)]"
          : "border border-slate-200 bg-white hover:border-[#1a4a82]/30 hover:shadow-[0_16px_48px_-12px_rgba(26,74,130,0.18)]"
    ...
    >
      ...
      <div className="relative z-10 flex flex-1 flex-col p-7">
        <p className={`mb-1 text-[10px] font-heading font-bold uppercase tracking-[0.2em] ${pkg.dark ? "text-sky-400/70" : "text-[#2d8fcf]"}`}>
          {pkg.note}
        </p>
        ...
        <h3 className={`mb-2 font-heading text-lg font-extrabold ${pkg.dark ? "text-white" : "text-[#0a1f3d]"}`}>{pkg.name}</h3>
        <p className={`mb-6 text-[13px] leading-relaxed ${pkg.dark ? "text-white/55" : "text-slate-500"}`}>{pkg.tagline}</p>
        ...
        <Link
          to="/book"
          className={`inline-flex items-center justify-center gap-2 rounded-full py-3 text-sm font-heading font-bold transition-all hover:-translate-y-0.5 ${
            pkg.dark
  ...
  <div id="pricing" className="section-target">
    <DocumentPage className="bg-slate-50">
      ...
      <div className="grid gap-5 lg:grid-cols-3 lg:items-center">
        {featured.map((pkg, i) => (
          <PricingCard key={pkg.name} pkg={pkg} i={i} />
        ))}
      </div>
